...
FEATURES_PATH = PROJECT_ROOT / "data" / "csv_data" / "financial_advisor_dataset.csv"
CFPB_PATH = PROJECT_ROOT / "data" / "csv_data" / "cfpb_credit_card.csv"
IC3_DIR = PROJECT_ROOT / "data" / "ic3_2024_csvs"
...
def api_health() -> dict | None:
    ...
    Only successful responses are cached; failures return None without
    poisoning the cache for a long window (API reload can take >2s).
    """
    try:
        r = requests.get(f"{API_BASE_URL}/api/health", timeout=5)
        ...
        # Bypass cache on failure so the next rerun retries immediately
        st.cache_data.clear()
        return None
    return None
...
def render_multimodal_tab():
            ...
            if st.button("📥 Index All Documents", key="btn_index_mm", type="primary"):
                ...
                with st.spinner(f"Indexing {len(st.session_state.mm_uploaded_files)} file(s)..."):
                    ...
                    with st.expander("Index Details", expanded=False):
                        for r in results:
                            emoji = "✅" if r.get("status") in ("indexed", "indexed_weak_ocr") else "❌"
                            detail = ""
                            if "pages" in r:
                                detail = (
                                    f" ({r['pages']} pages, {r.get('chunks', 0)} chunks"
                                    f", ocr_pages={r.get('ocr_pages', 0)})"
                                )
                            elif "rows" in r:
                                detail = f" ({r['rows']} rows, {len(r.get('columns', []))} columns)"
                            elif "ocr_text" in r or r.get("status") == "indexed_weak_ocr":
                                detail = f" (OCR chunks={r.get('chunks', 0)})"
                            st.write(f"{emoji} **{r['filename']}**: {r['status']}{detail}")

                    # Persist uploads to disk so API Vision/session OCR can find them
                    try:
                        from paths import UPLOADS_DIR
                        sess_dir = UPLOADS_DIR / session_id
                        sess_dir.mkdir(parents=True, exist_ok=True)
                        for fname, finfo in st.session_state.mm_uploaded_files.items():
                            (sess_dir / fname).write_bytes(finfo["bytes"])
                    except Exception as e:
                        st.caption(f"Note: could not mirror uploads to disk ({e})")
    ...
    with col_chat:
        ...
        if prompt := st.chat_input("Ask a question about your documents..."):
            if not st.session_state.mm_uploaded_files:
                st.warning("Upload files on the left first.")
                return
            if not st.session_state.mm_indexed:
                st.warning("Click **Index All Documents** before chatting so answers use your uploads.")
                return
                    ...
                    with st.spinner("Analyzing documents..."):
                        ...
                        if not is_chart_query or not chart_fig:
                            ...
                            visual_ask = any(
                                k in prompt.lower()
                                for k in [
                                    "image", "screenshot", "photo", "picture", "visual", "ocr",
                                    "tell me about", "describe this", "describe the",
                            ...
                            if visual_ask:
                                for fname, finfo in st.session_state.mm_uploaded_files.items():
                                    if fname.lower().endswith((".png", ".jpg", ".jpeg", ".bmp", ".webp")):
                                        b64 = base64.b64encode(finfo["bytes"]).decode("ascii")
                                        image_payload.append(f"data:image/png;base64,{b64}")
                            ...
                            try:
                                ...
                                # Fallback: Direct local RAG with relevance filter
                                try:
                                    rag = _get_multimodal_rag()
                                    # Ensure bytes are indexed into this session
                                    for fname, finfo in st.session_state.mm_uploaded_files.items():
                                        rag.index_file_bytes(fname, finfo["bytes"], session_id=session_id)
                                    answered = rag.answer_from_retrieval(
                                        prompt,
                                        n_results=6,
                                        session_id=session_id,
                                    )
                                    ...
                                    reply = (
                                        f"⚠️ Could not query documents: {str(e)}. "
                                        "Ensure files are uploaded and indexed."
                                    )
                        ...
                        if sources:
                            with st.expander("View Sources"):
                                for i, src in enumerate(sources):
                                    ...
                                    fname = src.get("filename") or meta.get("filename") or "Unknown"
                                    page = src.get("page") or meta.get("page")
                                    conf = src.get("confidence")
                                    page_bit = f", page {page}" if page is not None else ""
                                    conf_bit = f", conf={float(conf):.2f}" if conf is not None else ""
                                    st.markdown(f"**Source {i+1}: {fname}{page_bit}{conf_bit}**")
