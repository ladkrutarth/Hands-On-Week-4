...
class AdvisorChatRequest(BaseModel):
    ...
    approved: bool = Field(
        default=False,
        description="Human-in-the-loop approval for HITL-gated tools (e.g. challenge_auth).",
    )
...
class DNACompareRequest(BaseModel):
    ...
    approved: bool = Field(
        default=False,
        description="If true, auto-issue Challenge Auth when verdict is high deviation.",
    )
...
class DNAChallengeRequest(BaseModel):
    """HITL-gated Challenge Auth after a high DNA deviation."""
    user_id: str = Field(..., example="USER_0001")
    reason: str = Field(default="high_dna_deviation")
    approved: bool = Field(
        ...,
        description="Must be true — Challenge Auth will not run without human approval.",
    )
    session_id: Optional[str] = None


class DNAChallengeResponse(BaseModel):
    """Result of an approved Challenge Auth action."""
    user_id: str
    status: str
    challenge: Optional[Dict[str, Any]] = None
    message: str
...
class AuthLoginRequest(BaseModel):
    """Login request for Streamlit /auth/login."""
    username: str = Field(..., example="admin")
    password: str = Field(..., example="admin123!")


class AuthLoginResponse(BaseModel):
    """Login response including ADDF diversion info."""
    authenticated: bool
    session_id: Optional[str] = None
    diverted: bool = False
    message: str
...
class SecurityChatRequest(BaseModel):
    ...
    approved: bool = Field(
        default=False,
        description="Approve HITL tools such as challenge_auth.",
    )
